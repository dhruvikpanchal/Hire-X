...
/* POST /api/recruiters/feed/posts  body: { content } */
export const createRecruiterFeedPost = async (req, res) => {
  try {
    const me = req.user.id;
    if (!(await assertRecruiterUser(me))) {
      return res.status(403).json({ success: false, message: "Only recruiters can post" });
    }
    ...
    if (content.length > 5000) {
      return res.status(400).json({ success: false, message: "Post is too long (max 5000 characters)" });
    }
...
/* PATCH /api/recruiters/feed/posts/:id  body: { content } */
...
/* GET /api/recruiters/connections */
...
/* POST /api/recruiters/connections/request  body: { recruiterId } */
export const sendRecruiterConnectionRequest = async (req, res) => {
  try {
    ...
    if (!recruiterId || !isValidId(recruiterId)) {
      return res.status(400).json({ success: false, message: "Valid recruiterId is required" });
    }
    if (String(me) === String(recruiterId)) {
      return res.status(400).json({ success: false, message: "You cannot connect with yourself" });
    }

    const [iAmRec, theyAreRec] = await Promise.all([assertRecruiterUser(me), assertRecruiterUser(recruiterId)]);
    if (!iAmRec || !theyAreRec) {
      return res.status(403).json({ success: false, message: "Connections are only between recruiters" });
    }
...
/* POST /api/recruiters/connections/accept  body: { requestId } */
export const acceptRecruiterConnectionRequest = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: "Connection accepted", connection: row });
  } catch (error) {
    return res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
};

/* DELETE /api/recruiters/connections/:id — id = other user's User _id */
